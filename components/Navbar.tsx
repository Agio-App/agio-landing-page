  ...
  return (
    <motion.nav
      ...
      className={`fixed top-0 left-0 right-0 z-40 transition-all duration-300 ${
        isScrolled 
          ? 'bg-charcoal/80 backdrop-blur-md py-4 border-b border-white/5' 
          : 'bg-gradient-to-b from-charcoal/70 via-charcoal/30 to-transparent py-8'
      }`}
    >
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            className="fixed inset-0 z-50 bg-charcoal/95 backdrop-blur-md md:hidden"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          ...
          >
            <motion.div
              id="mobile-nav"
              className="h-full flex flex-col items-center justify-center gap-8 text-center text-white"
